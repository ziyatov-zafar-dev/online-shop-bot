...
import uz.zafar.onlineshoptelegrambot.botservice.SuperAdminTelegramBotFunction;
...
@Service
public class TelegramSuperAdminServiceImpl implements TelegramSuperAdminService {

    private final SuperAdminTelegramBotFunction functions;


    public TelegramSuperAdminServiceImpl(SuperAdminTelegramBotFunction functions) {
    ...
    public void handleUpdate(Map<String, Object> update, HttpServletRequest request) {
        ...
        if (text.equals("/start")) {
            ...
            if (text.startsWith("/start")) {
                String payload = "";
                if (text.length() > 6) {
                    payload = text.substring(7);
                }
                functions.example(chatId, payload);
                return;
            }
            functions.example(chatId, text);
        }
    }
}
